...
namespace Hexpoint.Blox.Hosts.Input
{
	internal class InputHost : IHost
	{
        ...
        private Position _leftMouseDownBlock;
        private Position _leftMouseUpBlock;
		...
		public void Update(FrameEventArgs e)
		{
            ...
            // Camera
            if (_game.Keyboard[Key.PageUp])  Game.cameraAngle = (float)Math.Min(Game.cameraAngle + (0.18 * (float)e.Time), 1.57);
            if (_game.Keyboard[Key.PageDown]) Game.cameraAngle = (float)Math.Max(Game.cameraAngle - (0.18 * (float)e.Time), 0.01);
		...
		private void Keyboard_KeyDown(object sender, KeyboardKeyEventArgs e)
		{
			...
			switch (e.Key)
			{
				case Key.C:
					if (isCtrlPressed) 
                    {
                        new PlayerOption(PlayerOption.OptionType.Creative, BitConverter.GetBytes(Config.CreativeMode ? 0 : 1)).Send();
                        if (Config.CreativeMode) Construction.Instance.Save("House");
                    }
					break;
				case Key.S:
		...
		private void OnMouseButtonDown(object sender, MouseButtonEventArgs e)
		{
			switch (e.Button)
			{
				case MouseButton.Left:
                    _leftMouseDownLocation = System.Windows.Forms.Cursor.Position;
                    //NetworkClient.SendAddOrRemoveBlock(BlockCursorHost.Position, Block.BlockType.Coal);
		...
		{
            if (_game.Mouse[MouseButton.Right] && _game.Mouse[MouseButton.Left] && _rightMouseDownLocation != System.Windows.Forms.Cursor.Position)
			{
				...
				{
					//mouse y changed, rotate camera pitch
                    ...
                    Game.cameraAngle = Game.cameraAngle - ((System.Windows.Forms.Cursor.Position.Y - _rightMouseDownLocation.Y) / 50f);
                    if (Game.cameraAngle > 1.57f) Game.cameraAngle = 1.57f;
                    if (Game.cameraAngle < 0.1f) Game.cameraAngle = 0.1f;
				}
				System.Windows.Forms.Cursor.Position = _rightMouseDownLocation;
			}
		}

        public void OnMouseWheelChanged(object sender, MouseWheelEventArgs e)
        {
            if (e.Delta < 0)
                Game.cameraRange = Math.Min (Game.cameraRange + 5.0f, Constants.MAX_CAMERA_RANGE);
            else if (e.Delta > 0)
                Game.cameraRange = Math.Max (Game.cameraRange - 5.0f, Constants.MIN_CAMERA_RANGE);
        }
		#endregion
