...
namespace Hexpoint.Blox.Hosts.World
{
	...
	public struct Position
	{
        ...
        public bool WithinBounds(Position pt1, Position pt2)
        {
            if (!BetweenPoints(X, pt1.X, pt2.X))
                return false;
            if (!BetweenPoints(Y, pt1.Y, pt2.Y))
                return false;
            if (!BetweenPoints(Z, pt1.Z, pt2.Z))
                return false;
        ...
        /// <summary>
        /// Returns true if the int is between 2 other ints. (inclusive)
        /// </summary>
        private bool BetweenPoints(int point, int pt1, int pt2)
        {
