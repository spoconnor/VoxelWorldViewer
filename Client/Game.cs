...
namespace Sean.WorldClient
{
   ...
   #region Static Properties
        ...
        public static CharacterHost CharacterHost;
        ...
        protected override void OnResize (EventArgs e)
        {
            ...
            if (Width < Constants.DEFAULT_GAME_WIDTH)
            {
                Width = Constants.DEFAULT_GAME_WIDTH;
                return;
            }
            if (Height < Constants.DEFAULT_GAME_HEIGHT)
            {
            ...
            foreach (var host in _hosts.Where(host => host.Enabled))
                host.Resize (e);
        }
        ...
        private static void UpdateFrustum ()
        {
            ...
            CameraCoords.Xf = Player.Coords.Xf - (float)Math.Cos (Player.Coords.Direction) * (float)Math.Sin (cameraAngle) * cameraRange;
            CameraCoords.Yf = Player.Coords.Yf + (float)Math.Cos (cameraAngle) * cameraRange;
            CameraCoords.Zf = Player.Coords.Zf - (float)Math.Sin (Player.Coords.Direction) * (float)Math.Sin (cameraAngle) * cameraRange;
            ModelView = Matrix4d.LookAt (
                CameraCoords.Xf, 
                CameraCoords.Yf, 
                CameraCoords.Zf, 
        ...
        {
            if (e.Time > Constants.UPDATE_TIME_ACCEPTABLE)
                return;
